terraform {
  backend "s3" {
    ...
    key            = "state/sandbox/go18talk-third"
  }
}

locals {
  talk_phase    = "go18talk-phase3"
  function_name = "toy"
  filepath      = "${format("%s/../../../third/%s.zip", path.module, local.function_name)}"
}

module "api" {
  source          = "../../modules/api"
  api_name        = "${local.talk_phase}"
  api_description = "third POC implementation of our API"
  environment     = "sandbox"
